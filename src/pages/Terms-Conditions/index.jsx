...
import { useTermsConditionData, useUpdateCms } from "./hooks/useTermsConditions";

const TermsConditionPage = () => {
  const { data: TermConditionData, isLoading, error, refetch } = useTermsConditionData();
  const { mutate: updateTermCondition, isPending: isUpdating } = useUpdateCms();
  const [isEditing, setIsEditing] = useState(false);
  const [editedContent, setEditedContent] = useState("");

  // Initialize edited content when data loads
  useEffect(() => {
    if (TermConditionData?.data?.description) {
      setEditedContent(TermConditionData.data.description);
    }
  }, [TermConditionData?.data?.description]);
  ...
  const handleSave = () => {
    ...
    const updateData = {
      cms_id: TermConditionData.data._id, // Use the _id from the response
      title: TermConditionData.data.title, // Keep the same title
      description: editedContent,
    };

    updateTermCondition(updateData, {
      onSuccess: (response) => {
        setIsEditing(false);
  ...
  const aboutContent = TermConditionData?.data;
