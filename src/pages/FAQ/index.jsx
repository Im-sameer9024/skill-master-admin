...
import { useFaqData, useUpdateCms } from "./hooks/useFaq";

const FaqPage = () => {
  const { data: FaqData, isLoading, error, refetch } = useFaqData();
  const { mutate: updateFaq, isPending: isUpdating } = useUpdateCms();
  const [isEditing, setIsEditing] = useState(false);
  const [editedContent, setEditedContent] = useState("");

  // Initialize edited content when data loads
  useEffect(() => {
    if (FaqData?.data?.description) {
      setEditedContent(FaqData.data.description);
    }
  }, [FaqData?.data?.description]);
  ...
  const handleSave = () => {
    ...
    const updateData = {
      cms_id: FaqData.data._id, // Use the _id from the response
      title: FaqData.data.title, // Keep the same title
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      {/* Last Updated Info */}
      {faqContent?.updatedAt && (
        <div className="text-sm text-gray-500 mt-4 text-right">
          Last updated:{" "}
          {new Date(faqContent.updatedAt).toLocaleDateString("en-US", {
            year: "numeric",
            month: "long",
            day: "numeric",
            hour: "2-digit",
